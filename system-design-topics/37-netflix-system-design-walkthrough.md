...
Netflix famously A/B tests everything: thumbnail images, title card order, play button placement, even the color of UI elements. This requires a percentage-based assignment system: each user is placed into a treatment bucket (hash of user_id + experiment_id modulo 100), and all services that render UI consult the experiment config to serve the appropriate variant. The experiment config is fetched at session start and cached client-side. Results (click-through rates, watch time) are logged per user per experiment and analyzed by a data pipeline. The missing design: how do you run 500 simultaneous A/B tests without combinatorial explosion in experiment interactions?

**DRM (Digital Rights Management)**
Content studios require Widevine (Android/Chrome) and FairPlay (Apple) DRM. The pipeline: video is encrypted with AES-128 at upload time using a Content Encryption Key (CEK). The CEK is stored in a Key Management Service (KMS), not in the CDN. When a client plays a video, it requests a license from the DRM License Server, which verifies the user's entitlement (active subscription, content available in their region) and returns the CEK encrypted for the client's platform-specific DRM module. The CDN serves only the encrypted video stream; the key is never in the CDN. This is the missing layer that explains why downloading a Netflix video doesn't give you a playable file.

**Viewing history and "continue watching"**
"Continue watching" requires recording playback position every 30 seconds across all devices with last-write-wins semantics and cross-device sync. The pipeline: client emits `playback_position` events to an events endpoint (not the main API) → Kafka → a position aggregation service → Cassandra (user_id + title_id → position, timestamp). On app open, the client fetches the latest position for each in-progress title. The challenge: a user pauses on their TV, the TV crashes. The last position event may not have been sent. Clients must flush buffered position events before the app closes; the server must accept delayed events and apply last-write-wins by timestamp.

### Critical Questions

> **"A user pauses mid-episode on their TV, then resumes on their phone. Playback position must sync within 2 seconds. Design this."**

When the user pauses on the TV, a `playback_paused` event includes the exact position. This event is sent immediately (not on the next 30-second heartbeat) and written to the position store (Cassandra). The phone app, when it opens, fetches the user's position for that title via a REST call — this is a simple Cassandra point read (< 10ms). The end-to-end latency from TV pause to phone showing the correct resume position is: TV event send (~200ms network) + Kafka processing (~50ms) + Cassandra write (~5ms) + phone fetch on app open (< 1s total). The 2-second requirement is easily met. The edge case: what if the TV loses network before the pause event is sent? The client buffers the event locally and retries when connectivity returns. If the user opens the phone before connectivity returns, they see the last confirmed position (potentially a few minutes behind). This is acceptable — a data loss of < 30 seconds of position is an acceptable trade-off for not blocking playback on network availability.

> **"Netflix shows different thumbnails for the same show to different users. How is this personalized A/B test implemented at scale?"**

Each title has multiple thumbnail variants stored in S3 and served via CDN. The user's assigned bucket for the thumbnail experiment is determined at session start and stored in their session context (a JWT or server-side session). The homepage service, when rendering a tile, fetches the experiment assignment from the session, selects the correct thumbnail variant URL, and returns it in the API response to the client. The CDN serves the chosen thumbnail — there is no per-user CDN URL (that would break caching). Instead, the API response tells the client which URL to load. Impression and click events are tagged with the variant ID and user bucket, and logged via a high-throughput events pipeline. The analysis runs in batch (daily): compare click-through rates and downstream watch time per variant per bucket.
