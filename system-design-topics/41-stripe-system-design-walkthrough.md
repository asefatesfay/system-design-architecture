...
Stripe's ML fraud scoring runs on every payment in < 100ms. Features include: card velocity (how many charges on this card in the last hour), device fingerprint, IP reputation, merchant's historical fraud rate, billing address mismatch, and card BIN (Bank Identification Number) risk profile. The model is a gradient-boosted classifier trained on labeled fraud data. At < 100ms latency, the model must be served from memory (not a database query loop) — all feature values are pre-computed and cached, with only the current transaction's features computed in real time.

**Multi-currency and FX**
Stripe operates in 135+ currencies. A charge in EUR to a USD merchant involves: (1) capturing the EUR amount, (2) locking an FX rate valid for the settlement window (typically T+2), (3) converting to USD for merchant payout. FX rates are fetched from a rate provider (e.g., ECB, Bloomberg) and cached with short TTLs (seconds). The locked rate is stored on the payment record. Rounding is handled with banker's rounding to prevent systematic bias across millions of micro-transactions. Currency conversion ledger entries are double-entry: debit EUR received, credit USD converted.

**Dispute and chargeback handling**
When a cardholder disputes a charge with their bank, the bank issues a chargeback to Stripe. Stripe withdraws the disputed amount from the merchant's balance, notifies them, and gives them a window (7–30 days depending on the card network) to submit evidence. The dispute is a long-running workflow: state machine with states `NEEDS_RESPONSE → EVIDENCE_SUBMITTED → UNDER_REVIEW → WON/LOST`. If lost, the merchant's Stripe balance is permanently debited. The evidence submission system accepts files (PDF, images) up to 50MB, stored in S3, and submitted to the card network via their API.

### Critical Questions

> **"A payment is in PROCESSING state for 30 seconds — the bank hasn't responded. The user refreshes and submits again. Walk me through exactly what your idempotency system does."**

The first request creates a payment record with `status=PROCESSING` and stores the idempotency key → payment_id mapping in Redis (TTL: 24 hours). The bank call is in flight. The user refreshes — the client resends with the same idempotency key. The Payment Service looks up the idempotency key in Redis, finds an existing payment_id, and returns the current status of that payment (`PROCESSING`) without creating a new record or making a new bank call. The user sees "Payment in progress." When the bank responds (success or failure), the payment record is updated and the idempotency key's cached response is updated to reflect the final state. If the user refreshes again after the bank responds, they get the final status immediately from the idempotency cache. The idempotency key prevents the double-charge regardless of how many times the user hits refresh.

> **"Stripe must reconcile with Visa/Mastercard every night — your records vs. theirs. How do you detect and resolve discrepancies?"**

Nightly reconciliation: Stripe downloads the settlement file from Visa/Mastercard (a structured file listing all transactions they recorded for the day) and compares it against Stripe's internal transaction ledger. Discrepancies: (1) **In Stripe's ledger but not in Visa's file** — the transaction may not have settled; Stripe re-checks the authorization status. (2) **In Visa's file but not in Stripe's ledger** — this is a serious error (Visa charged a card but Stripe has no record); triggers an immediate alert and investigation. (3) **Amount mismatch** — the authorized amount differs from the settled amount (common for partial refunds or tip adjustments in restaurants). All discrepancies are written to a reconciliation exceptions table and assigned to an automated resolution workflow. Unresolvable exceptions escalate to a human analyst. The system must process millions of records in the nightly window (typically 2–6 AM) — run as a parallel Spark job, one partition per card network per currency.
