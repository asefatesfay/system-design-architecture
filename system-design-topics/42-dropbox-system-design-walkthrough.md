...
**Ransomware recovery**
Ransomware replaces thousands of files with encrypted versions in rapid succession — a detectable abnormal write pattern. Dropbox detects this via: (1) **Write velocity anomaly** — a client replacing > N files/minute triggers a flag (normal human editing is much slower). (2) **Content entropy analysis** — encrypted files have near-maximum byte entropy (they look like random data). Dropbox samples file content entropy on upload; a sudden shift to high-entropy files across many file types triggers an alert. (3) **Version history** — Dropbox retains 30–180 days of file versions (product-tier dependent). Recovery: the user or support agent selects a point-in-time before the attack and restores all files to that snapshot — which is a bulk "revert to version X" operation across potentially thousands of files.

**Large file handling with resumable upload**
A 100GB video upload with a network interruption at 60GB must not restart from 0. Implementation: the client splits the file into fixed-size chunks (e.g., 4MB each). Each chunk is uploaded individually with a `chunk_index` and file `upload_session_id`. The server stores received chunks in a temp location and tracks which chunk indices have been received (a bitmask in Redis keyed by `upload_session_id`). On reconnect, the client queries which chunks the server has received, and resumes from the first missing chunk. Final assembly: when all chunks are present, the server assembles them in order, computes the block hash, and completes the upload. This is similar to S3's multipart upload API — Dropbox built an equivalent.

**Selective sync / smart sync**
Files that appear in the Dropbox folder as stubs (showing metadata, not content) use a virtual file system (VFS) driver — a kernel extension on macOS/Windows. The VFS driver intercepts file open calls: if the file is a stub (not locally downloaded), it triggers a download from Dropbox before returning the file data to the calling application. The user's app sees no difference — the file "opens" normally, just with a brief network delay. The design challenge: the VFS driver must handle concurrent opens, partial reads, and app crashes mid-download without corrupting the local file cache.

### Critical Questions

> **"Two users edit the same Word document offline for 2 hours with no network. Both come online simultaneously. How does Dropbox resolve the conflict without losing either user's changes?"**

Dropbox does not resolve conflicts automatically for binary files like Word documents — it cannot merge arbitrary binary formats. The resolution: both versions are preserved. Dropbox computes a hash of each uploaded file. If the server-side version has changed since the client last synced (version vector mismatch), Dropbox creates a "conflicted copy": User 1's version is saved at the original filename, User 2's version is saved as `filename (User 2's conflicted copy YYYY-MM-DD).docx`. Both files appear in the Dropbox folder. Neither version is lost. The user must manually review and merge them — Dropbox has no semantic understanding of the content. For text-based files (Dropbox Paper, code files), Dropbox can perform a 3-way merge (base version + two diverged versions) using standard diff algorithms.

> **"A user deletes their entire Dropbox folder locally. How do you prevent this from being treated as a mass deletion and propagated to the cloud?"**

The local Dropbox client monitors the sync folder via OS file-system events (inotify on Linux, FSEvents on macOS, ReadDirectoryChangesW on Windows). A mass deletion event — deleting hundreds of files within seconds — triggers a heuristic: if > N% of the tracked file set is deleted in < T seconds, the client pauses sync and shows a warning: "It looks like you deleted most of your Dropbox. Was this intentional?" The user must explicitly confirm before the deletions are propagated. This is a UX safety gate, not a technical lock — the client-side heuristic is the last line of defense. Additional protection: cloud-side, Dropbox retains all deleted files in a trash/version history for 30 days regardless; even if the propagation occurs, the files are recoverable.
